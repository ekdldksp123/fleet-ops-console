...
![Fleet Ops Console — 2,000대 플릿 실시간 관제 화면](docs/screenshot.png)

<sub>2,000대 · 서버 tick 100ms · 이진·워커 수신 경로 · Canvas 2D 렌더. 우하단 계측
오버레이가 캡처 시점의 **FPS 60.1 / 최장 프레임 17.6ms / 델타 삼킴 0.10ms / 프레임당
43KB** 를 그대로 보여준다. 이 이미지는 `node scripts/screenshot.mjs` 로 재현된다 —
숫자를 손으로 적어 넣지 않았다.</sub>
...
yarn test            # Vitest 단위 테스트 (94개)
yarn test:e2e        # Playwright E2E
yarn build           # 프로덕션 빌드
```

측정·캡처용 스크립트는 `scripts/` 에 있다. 셋 다 프로덕션 빌드를 3210 번 포트에
직접 띄우므로 `yarn build` 를 먼저 돌려야 한다(개발 중인 3000 번 dev 서버와 겹치지
않게 포트를 분리해 두었다).

```bash
node scripts/benchmark.mjs        # FPS 측정 → 아래 "벤치마크" 절의 표
node scripts/screenshot.mjs       # README 스크린샷 재생성
node scripts/verify-reconnect.mjs # 서버를 죽였다 살려 재연결 확인
```
...
app/
  ...
  api/fleet/stream/route.ts         Route Handler — SSE (JSON)
  api/fleet/binary/route.ts         Route Handler — 이진 스트림 (같은 데이터, 다른 인코딩)
...
다만 텔레메트리 쪽은 SSE의 한계에 부딪혔다 — **텍스트 전용**이라 이진 프레임을 실을 수
없다. 그래서 이진 경로는 `fetch` 스트림으로 따로 뒀고, 그 대가로 자동 재연결을 직접
구현했다. 아래 "6. 서버가 이진 프레임을 보낸다" 참고.
...
![상세 라우트 — 지도·목록·경보 레일은 그대로 두고 패널만 바뀐다](docs/screenshot-detail.png)

<sub>`/fleet/RB-00042`. 지도·목록·경보 레일은 히어로 이미지와 같은 인스턴스 그대로다 —
라우트만 바뀌었다. 선택된 로봇에만 경로 궤적(주황 선)이 그려지고, 목록에서도 해당 행이
강조된다. 지도 위에 겹쳐 뜬 패널이 `children` 슬롯으로 들어온 부분이고, 오른쪽 경보
레일은 `@alerts` 슬롯이다.</sub>
...
1. `FLEET_SIZE`를 바꿔가며 `yarn build && yarn start`
2. 각 조건에서 Canvas 모드로 30초 방치 후 FPS 기록 (**before**)
3. WebGL 모드로 전환, 동일하게 30초 후 기록 (**after**)
4. 지도를 팬·줌하며 최저 FPS도 함께 기록 — 평균만 보면 스터터가 숨는다

같은 절차를 `scripts/benchmark.mjs` 에 넣어 뒀다. 아래 두 표는 전부 이걸로 뽑은
값이다.

```bash
yarn build
# 렌더 경로 표 — 조건당 3회, 중앙값
node scripts/benchmark.mjs --repeat 3 --seconds 12 --warmup 8
# 최적화 단계 표 — 옛 커밋을 체크아웃해서 (아래 절 참고)
node scripts/benchmark.mjs --sizes 20000 --modes canvas --dev \
     --repeat 3 --seconds 9 --warmup 8 --allow-slow
```

> ⚠️ **브라우저 창이 화면에 보이는 상태여야 한다.** Chrome 은 보이지 않는 창의 rAF 를
> 억제한다. 같은 페이지·같은 부하(2,000대)에서 창 상태만 바꿔 실측한 값:
>
> | 창 상태 | 평균 FPS | 최장 프레임 |
> | --- | --- | --- |
> | 헤드리스 | 10.5 | 120.7ms |
> | 헤드풀, 다른 창에 가려짐 | 14.2 | 122.2ms |
> | 헤드풀, 맨 앞 | **60.1** | **29.2ms** |
>
> 10.5 는 앱 성능이 아니라 **창이 안 보인다는 사실**을 재고 있는 값이다. 서버 tick 을
> 100ms → 16ms 로 올려도 그대로였으니 데이터 주기 탓도 아니다. 그래서 스크립트는
> 헤드풀로 띄우고 창을 맨 앞으로 가져온다 — 측정 중에 다른 창으로 덮으면 값이 무너진다.
> FPS 를 CI 에서 재려던 계획을 접은 이유이기도 하다.
...
> `scripts/benchmark.mjs` 는 시작할 때 실제 렌더러 문자열을 찍는다.

### 결과 표 — 렌더 경로

측정 환경: **프로덕션 빌드**, 1400×900, 이진·워커 수신 경로, 서버 tick 100ms, 초기
뷰(전체 줌아웃), 워밍업 8초 → 12초 측정, 헤드풀 Chrome 맨 앞, **조건당 3회 측정의
중앙값**(괄호는 3회 범위). WebGL 렌더러는 `ANGLE (Apple, ANGLE Metal Renderer:
Apple M4)` — 하드웨어 가속이 켜진 상태다.

| 로봇 대수 | Canvas 평균 | Canvas 최저 (범위) | Canvas 최장 | WebGL 평균 | WebGL 최저 (범위) | WebGL 최장 |
| --- | --- | --- | --- | --- | --- | --- |
| 500 | 60.0 | 50.3 (33.4~56.5) | 19.9ms | 60.0 | 44.8 (44.4~55.6) | 22.3ms |
| 1,000 | 60.1 | 51.5 (40.2~56.2) | 19.4ms | 60.4 | 49.5 (25.1~50.0) | 20.2ms |
| 2,000 | 60.0 | 29.9 (19.8~53.5) | 33.4ms | 60.0 | 43.5 (32.9~55.2) | 23.0ms |
| 5,000 | 60.1 | 29.9 (24.3~34.5) | 33.4ms | 60.0 | 41.5 (34.8~56.2) | 24.1ms |
| 20,000 | **57.9** | **29.5** (29.2~29.9) | **33.9ms** | **60.0** | **56.5** (56.5~56.8) | **17.7ms** |

읽는 법 — **평균 FPS 로 보면 차이가 없고, 최저 FPS 로 보면 있다.**

1. **평균 FPS 로는 아무것도 안 보인다.** 500대부터 20,000대까지 전부 60 에 붙는다.
   Canvas 가 20,000대에서 57.9 로 처음 떨어지지만 3.5% 차이다. 평균만 적었다면
   "렌더 경로는 상관없다" 는 결론을 내렸을 것이다.

2. **차이는 최저 FPS 와 최장 프레임에 있다.** 20,000대에서 Canvas 29.5 / 33.9ms 대
   WebGL 56.5 / 17.7ms — 거의 두 배다. 그리고 3회 측정 범위가 **겹치지 않는다**
   (29.2~29.9 대 56.5~56.8). 노이즈가 아니다. Canvas 는 2,000대부터 최장 프레임이
   33ms 대 — **정확히 프레임 두 개** — 에 고정되고 부하가 늘어도 그대로다. 주기적으로
   프레임 하나를 건너뛴다는 뜻이다. WebGL 은 20,000대에서 오히려 17.7ms 로 가장 좋다.

3. **1회 측정으로는 이 결론에 못 갔다.** 처음 조건당 1회로 재고 "차이 없음" 이라고
   적었는데, 그때 5,000대 WebGL 에 최저 6.0 / 최장 167ms 같은 값이 섞여 있었다. 원인은
   앱이 아니라 **측정 중 창이 앞에서 밀린 것**이었다. 그래서 하네스가
   `document.hasFocus()` 와 rAF 억제 서명(평균 30 FPS 미만 + 최장 80ms 초과)으로 그런
   측정을 버리고 다시 재게 했다. 위 표를 뽑은 30회 측정에서 버려진 건 0회다.

4. **저부하 구간(500~1,000대)에서는 정말 차이가 없다.** 두 경로 모두 평균 60, 최장
   20ms 안쪽이다. 기본값을 Canvas 로 둔 이유다 — 이 구간에서 얻는 것 없이 아래
   "4. Canvas → WebGL 전환" 절의 제약(팬·줌 중 정지, 컬링 없음, 히트 디텍션 비용)을
   지불할 이유가 없다. 부하가 20,000대 급이면 토글로 바꾸면 된다.

### 결과 표 — 최적화 단계

단계별 커밋을 하나씩 체크아웃해 측정한다. `scripts/benchmark.mjs` 를 커밋 바깥으로
꺼내 두면 옛 커밋에서도 같은 하네스로 잴 수 있다 — 그 시점에 없던 수신 경로 토글과
오버레이 행은 스크립트가 알아서 건너뛴다. `--allow-slow` 가 필요하다: 하네스는
"평균 30 FPS 미만 + 최장 프레임 80ms 초과" 를 창이 가려진 측정으로 보고 버리는데,
0단계는 **진짜로** 그 구간이라 그냥 두면 유효한 데이터가 버려진다.

```bash
git log --oneline                                    # 단계별 커밋 확인
cp -r scripts /tmp/bench                             # 하네스를 빼 둔다
git checkout <커밋> && cp -r /tmp/bench scripts
node scripts/benchmark.mjs --sizes 20000 --modes canvas --dev \
     --seconds 9 --warmup 8 --repeat 3 --allow-slow  # 줌인 행은 --zoom in
```
...
측정 환경: 설치된 Chrome **헤드풀·창 맨 앞**, **dev 빌드**, `FLEET_SIZE=20000`,
뷰포트 1400×900, Canvas 모드, SSE·메인 수신 경로(그 시점에 있던 유일한 경로),
워밍업 8초 → 9초 측정, **조건당 3회 측정의 중앙값**. dev 빌드라 절대값은 프로덕션
보다 나쁘다 — 상대 비교용으로만 쓸 것.

| 단계 | 커밋 | 평균 FPS | 최저 FPS (3회 범위) | 최장 프레임 |
| --- | --- | --- | --- | --- |
| 0. 소박한 루프 (before) | `1ad9832` | **22.8** (16.9~25.8) | 8.6 (3.0~10.0) | 116.9ms |
| 1. silent 갱신 + `changed()` 1회 | `8ac7157` | **50.2** | 20.0 (20.0~29.1) | 50.0ms |
| 2. + rAF 코얼레싱 | `04bbc64` | 50.6 | 20.0 (12.0~29.4) | 50.0ms |
| 3. + 뷰포트 컬링 (줌아웃) | `79be4c0` | 52.3 | 29.2 (19.6~29.2) | **34.3ms** |
| 3. + 뷰포트 컬링 (줌인) | `79be4c0` | 59.9 | 46.5 (44.1~52.6) | **21.5ms** |

줌인 행은 확대를 **워밍업 전에** 끝낸다. 조작 중의 줌 애니메이션 스터터가 최저 FPS
에 섞이면 컬링 효과가 그 밑에 가려지기 때문이다.

읽는 법 — 표에서 실제로 배울 것은 네 가지다.

1. **1단계가 개선의 거의 전부다.** 22.8 → 50.2 FPS. 나머지 두 단계는 이 조건에서
   측정 오차 안에 있다. "최적화 4개를 했으니 4배 좋아졌다" 가 아니다.

2. **before 는 재현이 안 된다.** 같은 커밋을 3회 측정해 22.8 / 25.8 / 16.9 가 나왔고,
   최장 프레임은 100 / 117 / 334ms 로 3배 넘게 벌어졌다. 프레임 예산을 초과한
   시스템은 붕괴가 자기 강화되기 때문이다(프레임이 늦으면 델타가 쌓이고, 쌓인 델타가
   다음 프레임을 더 늦춘다). **병목 상태의 측정값은 불안정하다**는 것 자체가 관찰
   결과다. 1단계 이후에는 50.1 / 50.2 / 50.2 처럼 세 번이 거의 붙는다 — 그래서
   평균 FPS 의 3회 범위는 0단계 행에만 적었다. 나머지 행은 붙어 있어서 적을 게 없다.

3. **2단계는 예상대로 안 나타난다.** 50.2 → 50.6, 최장 프레임은 둘 다 50.0ms 로
   같다. 설계상 이 조건(10Hz, 포그라운드 탭)에서는 이득이 없다. 근거는 아래
   "2. rAF 코얼레싱" 절에 적어 두었다. 효과를 보려면 `FLEET_TICK_MS` 를 16 아래로
   내리거나 탭을 백그라운드로 보내야 한다.

4. **3단계는 평균 FPS 보다 최장 프레임에서 보인다.** 평균은 50.6 → 52.3 으로 3%
   오르는 데 그치는데, 최장 프레임은 50.0 → 34.3ms(줌아웃), 줌인에서는 21.5ms 까지
   줄어든다. 최저 FPS 도 20.0 → 29.2 → 46.5 으로 단계마다 올라간다. 이게 컬링이
   실제로 일했다는 증거다. **평균만 보면 놓친다.**
...
5단계(Web Worker)는 **조건부 이득**으로 결론이 났다 — 렌더가 포화된 구간에서는 오히려
손해였다. 6단계(서버 이진 전송)가 그 원인(CPU 경쟁)을 없애고 모든 구간에서 이겼고,
그래서 **수신 경로의 기본값은 `binary`** 다. 측정 없이 "최적화를 다 했다"고 읽으면 안
되는 과정이라 실패한 가설까지 그대로 적어 두었다.
...
이 제약을 지불할 값이 나오는 지점은 20,000대다. 그 부하에서 최저 FPS 29.5 → 56.5,
최장 프레임 33.9 → 17.7ms 로 벌어진다(위 "결과 표 — 렌더 경로"). 반대로 5,000대
아래에서는 두 경로가 구별되지 않으므로, 제약만 남고 얻는 게 없다.
...
그래서 이 단계만으로는 **기본값으로 올릴 수 없었다.** "켜면 빨라지는 스위치"가 아니라
특정 구간에서만 값을 하는 도구였다. 전제조건을 확인하라던 README의 경고가 정확히 이
상황을 막으려던 것이었다.

(6단계에서 원인이 GC가 아니라 **CPU 경쟁**임이 드러나고, 파싱 자체를 없애자 모든
구간에서 이겼다. 최종 기본값은 `binary`다.)

#### 버퍼 풀링 — 할당은 0이 됐지만 FPS는 안 움직였다

`transfer`로 넘긴 ArrayBuffer는 워커에서 분리되므로 재사용할 수 없다. 그대로 두면
프레임마다 네 개를 새로 할당한다 — 60,000대 기준 약 1.5MB × 10Hz = **초당 15MB**,
20,000대 @50Hz면 초당 약 16MB다. 그래서 메인 스레드가 다 쓴 버퍼를 워커로
되돌려주고(`recycle` 메시지, 이쪽도 `transfer`) 워커가 풀에 넣어 재사용한다.

플릿 크기가 고정이므로 크기별 풀을 관리하지 않고 **최대 용량으로 잡고 앞부분만
쓴다.** 재사용 시 뒷부분에 옛 데이터가 남지만 `count`만큼만 읽으므로 새지 않는다.
그게 풀링의 유일한 위험이라 `tests/frame-codec.test.ts`가 큰 프레임 → 작은 프레임
순서로 같은 버퍼를 쓰고 옛 좌표가 섞이지 않는지 확인한다.

반납 시점이 정확성의 핵심이다. **다 읽은 직후, 리스너 호출 전**이다.
더 일찍 반납하면 분리된 버퍼를 읽어 로봇이 좌표 (0,0)으로 순간이동하는데 에러는
안 난다. 리스너 뒤로 미루면 렌더가 끝날 때까지 버퍼가 묶여 워커가 풀을 비어 있다고
보고 새로 할당한다.

동작 확인은 계측 오버레이의 **"버퍼 할당"** 으로 한다. 워커 모드에서만 나오고,
연결 직후 몇 번 오르고 멈춰야 정상이다.

| 조건 | 삼킴(메인) | FPS | 버퍼 할당 |
| --- | --- | --- | --- |
| 60,000 @10Hz 줌아웃 | 12.00 → 2.10ms | 15.9 → 9.7 | **0** |
| 20,000 @50Hz | 2.20 → 0.50ms | 25.2 → 29.0 | **0** |
| 60,000 @10Hz 줌인 | 11.40 → 1.60ms | 59.8 → 60.0 | **0** |

**풀링은 설계대로 동작했다 — 할당이 0이 됐다. 그런데 FPS는 어느 구간에서도
바뀌지 않았다.**

그래서 위에서 세운 가설("악화 원인은 프레임당 1.3MB 할당의 GC 압력")은 **틀렸다.**
할당이 0인데도 60,000대 줌아웃은 그대로 9.7이다. 남는 설명은 **CPU 경쟁**이다 —
메인 스레드가 렌더로 이미 포화된 상태에서 워커가 100ms마다 2MB를 파싱하며 코어를
점유하면, 렌더러가 쓸 CPU가 줄어든다. 스레드를 늘려도 코어가 늘지는 않는다.

풀링은 그래도 유지한다. 같은 결과를 내면서 초당 15MB의 쓰레기를 만들지 않고,
긴 세션이나 저사양 기기에서는 메모리 압력 차이가 드러날 수 있다. 다만 **이 기기의
FPS로는 이득이 측정되지 않았다**는 사실을 그대로 적어 둔다 — 안 그러면 다음 사람이
"풀링했으니 빨라졌겠지" 로 읽는다.

상태까지 워커로 옮기려면 `SharedArrayBuffer`가 필요한데, COOP/COEP 헤더를 요구하고
그러면 OSM 타일이 CORP 없이는 막힌다 — 지금 구조에서 감당할 트레이드오프가 아니다.

### 6. 서버가 이진 프레임을 보낸다 ✅ — 파싱을 없앤다

5단계는 "JSON을 **어디서** 파싱할까"를 다뤘다. 옮겨도 파싱은 사라지지 않았고, 렌더가
포화된 구간에서는 워커가 코어를 점유해 FPS가 오히려 떨어졌다. 진짜 해법은 애초에
JSON을 쓰지 않는 것이다.

**SSE는 텍스트 전용**이라 이진을 실으려면 base64가 필요하다(대역폭 +33%, 디코딩 비용).
그래서 `fetch` + `ReadableStream`으로 받는다. Route Handler는 이미
`ReadableStream<Uint8Array>`를 반환하고 있었으므로 텍스트 인코딩만 걷어냈다.

`/api/fleet/binary`를 **별도 라우트**로 뒀다. 하나로 합쳐 쿼리로 분기하면 두 경로가
얽혀서 "이 차이가 인코딩 때문인지 분기 때문인지" 를 말할 수 없다.

#### 대가: EventSource가 해주던 일을 떠안는다

| 잃은 것 | 직접 구현 |
| --- | --- |
| 프레임 경계 (`\n\n`) | 헤더의 길이 접두로 재조립 (`lib/binary-feed.ts`) |
| 자동 재연결 | 지수 백오프 재연결 루프 |
| `Last-Event-ID` | 불필요 — 델타는 자기완결적이고 seq만 단조 증가하면 된다 |

이 프로젝트가 SSE를 고른 이유가 바로 그 자동 재연결이었으므로, 이건 되돌리는 선택이
아니라 **교환**이다.

#### 포맷에서 신경 쓴 것

**정렬.** `Float64Array` 뷰는 8바이트 정렬을 요구한다. 헤더를 32바이트로 잡고
`lonLat`을 페이로드 맨 앞에 둬서 유일하게 확실히 정렬된 위치를 쓴다.

**청크 경계.** 스트림 청크는 프레임과 무관하게 잘려 온다. 그래서 받은 바이트에 타입
배열 뷰를 직접 얹지 않고 **바이트 단위로** 정렬된 풀 버퍼에 복사한다. 원본 정렬과
무관하게 안전하고 요소별 루프 없이 memcpy 네 번으로 끝난다. 복사가 남는다는 점은
정직하게 인정해야 하지만, 1.4MB memcpy는 같은 크기의 `JSON.parse`보다 자릿수가 다르게 싸다.

**엔디언.** 헤더 스칼라는 `DataView`로 명시해 읽지만 페이로드는 바이트 복사 후 타입
배열로 해석하므로 **플랫폼 엔디언**을 따른다. 빅 엔디언 기기에서는 좌표가 쓰레기가
되므로, 감지해서 JSON 경로로 물러난다.

**백프레셔.** 이진 프레임은 60,000대 기준 1.3MB다. 클라이언트가 느릴 때 그냥 밀어
넣으면 큐가 무한히 자란다. `controller.desiredSize`가 0 이하면 그 프레임을 버린다 —
델타는 다음 프레임이 어차피 최신 상태를 담으므로 밀린 과거를 배달할 이유가 없다.

#### 결과 — 세 경로 A/B/C

| 조건 | 경로 | 삼킴(메인) | FPS | 페이로드 |
| --- | --- | --- | --- | --- |
| 60,000 @10Hz **줌아웃** | SSE·메인 | 11.70ms | 16.7 | 2,034KB |
| | SSE·워커 | 1.80ms | **9.7** | 2,020KB |
| | **이진·워커** | 2.80ms | **19.4** | **1,285KB** |
| 60,000 @10Hz 줌인 | SSE·메인 | 11.30ms | 58.0 | 2,024KB |
| | SSE·워커 | 1.60ms | 60.0 | 2,018KB |
| | **이진·워커** | 1.60ms | 60.0 | **1,284KB** |
| 20,000 @50Hz | SSE·메인 | 2.10ms | 25.2 | 673KB |
| | SSE·워커 | 0.50ms | 29.0 | 673KB |
| | **이진·워커** | 0.50ms | 29.0 | **429KB** |

**이진은 세 구간 모두에서 메인보다 나쁘지 않은 유일한 경로다.** 그리고 워커 JSON이
망가뜨렸던 구간(렌더 포화, 9.7)을 **19.4로 되살렸다** — 베이스라인 16.7보다도 높다.

그 이유가 앞선 진단을 확인해 준다. 워커 JSON이 손해였던 원인은 GC가 아니라 **CPU
경쟁**이었다. 이진 경로에서는 워커가 파싱 대신 memcpy만 하므로 코어를 거의 쓰지
않고, 그래서 렌더러와 다투지 않는다. **스레드를 늘리는 게 아니라 일을 없애는 것이
답이었다.**

페이로드는 세 구간 모두 **약 37% 작다.** JSON의 대괄호·쉼표·부동소수 십진 표기가
사라진 몫이다.

#### 기본값은 `binary`

렌더 모드(Canvas 기본)와 방침이 다르다. 렌더는 GPU 유무에 따라 결과가 뒤집혀서
기본값을 정할 수 없지만, **수신 경로는 측정이 끝났다** — 이진이 세 구간 모두에서
메인보다 나쁘지 않고 대역폭이 37% 적다. 측정이 끝난 축을 일부러 나쁜 쪽에 고정해 둘
이유가 없다.

before 비교는 우상단 토글로 언제든 할 수 있다. 데이터가 동일하므로 공정하다.

물러날 길도 있다. `Worker`를 못 쓰는 환경이나 빅 엔디언 플랫폼에서는 `connect()`가
자동으로 메인 경로로 내려가고, 워커가 죽으면 메인으로 폴백한다.

기본값을 바꾸기 전에 재연결을 먼저 검증했다 — 아래.
...
그래서 세 층으로 나눠 검증했다. 가장 거친 층(서버 프로세스 자체를 죽이는 경우)은
`scripts/verify-reconnect.mjs` 로 따로 돌린다.
...
세 층 모두 통과한다. 그래서 기본값을 `binary` 로 올렸다(`store/fleet-store.ts`).
렌더 모드와 달리 이 축은 측정이 끝났고, 재연결까지 확인했기 때문이다.
...
- `tests/binary-feed.test.ts` — 재연결 루프. `fetch` 를 스텁해 연결 거부·503·스트림
  종료를 결정적으로 재현한다. 실제 서버를 죽이고 살리는 방식도 되지만(지금은
  `scripts/verify-reconnect.mjs`) `pkill` 과 8초 대기가 섞여 CI 에 넣을 만큼
  결정적이지 않아서, 자동 검증은 스텁으로 하고 그 스크립트는 수동 확인용으로 남겼다.
