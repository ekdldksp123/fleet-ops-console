...
```bash
yarn build
node scripts/benchmark.mjs                                    # 렌더 경로 표
...
뷰(전체 줌아웃), 워밍업 8초 → 15초 측정, 헤드풀 Chrome 맨 앞, 조건당 1회.
WebGL 렌더러는 `ANGLE (Apple, ANGLE Metal Renderer: Apple M4)` — 하드웨어 가속이
켜진 상태다.

| 로봇 대수 | Canvas 평균 | Canvas 최저 | Canvas 최장 | WebGL 평균 | WebGL 최저 | WebGL 최장 |
| --- | --- | --- | --- | --- | --- | --- |
| 500 | 60.0 | 56.5 | 17.7ms | 60.0 | 28.2 | 35.4ms |
| 1,000 | 60.0 | 56.5 | 17.7ms | 60.0 | 56.2 | 17.8ms |
| 2,000 | 59.9 | 30.0 | 33.3ms | 60.0 | 56.5 | 17.7ms |
| 5,000 | 60.0 | 46.9 | 21.3ms | 59.4 | 6.0 | 167.4ms |
| 20,000 | 57.7 | 20.2 | 49.6ms | 59.9 | 32.6 | 30.7ms |

읽는 법 — **이 표의 결론은 "차이가 없다" 다.** 그래서 표를 지우지 않고 남겨 둔다.

1. **평균 FPS 는 500대부터 20,000대까지 전부 상한(60)에 붙는다.** 프로덕션 빌드에서
   렌더 경로는 이 구간의 병목이 아니다. WebGL 로 바꿀 근거가 평균 FPS 에는 없다.
   20,000대에서 Canvas 가 57.7 로 처음 상한에서 떨어지지만 3.7% 차이다.

2. **최저 FPS·최장 프레임으로는 우열을 못 가린다 — 단발 측정 편차가 너무 크다.**
   같은 WebGL 경로가 500대에서 최저 28.2, 1,000대에서 56.2, 5,000대에서 6.0(최장
   167ms)이 나왔다. 부하와 단조 관계가 아니므로 렌더 경로 탓이 아니라 타일 로딩·
   레이어 초기화 스터터가 잡힌 값으로 본다. **조건당 1회 측정으로 최저 FPS 를 비교하는
   건 틀린 방법이다.** 중앙값을 쓰려면 조건마다 3회 이상 반복해야 하는데 아직 안 했다.

3. 그래서 이 프로젝트에서 최적화의 근거가 되는 건 이 표가 아니라 아래 **"결과 표 —
   최적화 단계"** 다. 렌더 경로를 바꿔서 생기는 차이보다 **갱신 루프를 바꿔서 생기는
   차이가 훨씬 크다.** 4단계(WebGL)를 "성능 개선" 이 아니라 "1~3단계가 렌더 경로와
   무관하게 성립한다는 확인" 으로 적어 둔 이유다.
...
측정 환경: 설치된 Chrome, **dev 빌드**, `FLEET_SIZE=20000`, 뷰포트 1400×900,
Canvas 모드, 워밍업 후 9초 측정. dev 빌드라 절대값은 프로덕션보다 나쁘다 — 상대
비교용으로만 쓸 것.

> ⚠️ 이 표의 측정 환경을 처음에 "헤드리스로 구동" 이라고 적어 뒀는데, 그건 틀린
> 기록이다. 위 창 가시성 경고대로 헤드리스에서는 2,000대에서도 10.5 FPS 밖에 안
> 나오므로 **57.7 이 헤드리스 값일 수 없다.** 현재 코드로 다시 재면 20,000대·Canvas
> 에서 57.7 FPS(프로덕션, 헤드풀·맨 앞)가 나온다 — 값 자체는 창이 보이는 조건의
> 값이고, 환경 표기만 잘못돼 있었다. 1단계 행이 현재 코드에 해당하므로 재현되는
> 셈이지만, 0~3단계 행은 옛 커밋을 체크아웃해 `scripts/benchmark.mjs` 로 다시 재는
> 게 정확하다.
